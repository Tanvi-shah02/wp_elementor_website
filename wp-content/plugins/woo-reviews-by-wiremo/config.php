...
function display_wiremo_location_widget()
{
    if (current_user_can('administrator')) {
        $options = array("name" => "Location for widget",
            ...
            "std" => "Footer",
            "type" => "select",
            "options" => array("Tab" => "Tab", "Footer" => "Footer"));
        ?>
...
function wiremo_enable_automated_review_request() {
    if (current_user_can('administrator')) {
        ?>
        <input <?php if(!(get_option('wiremo_widget_display'))) echo " disabled "; ?> name="wiremo_automated_review_request" id="wiremo_automated_review_request" type="checkbox"
...
function display_wiremo_email_templates()
{
    if (current_user_can('administrator')) {
        ...
        <select <?php if(!(get_option('wiremo_widget_display'))) echo " disabled "; ?> <?php if(!(get_option('wiremo_automated_review_request'))) echo " disabled ";  ?> class="of-input" name="<?php if (isset($options['id'])) {
            echo $options['id'];
        } ?>" id="<?php if (isset($options['id'])) {
            echo $options['id'];
        } ?>">
...
if(!function_exists("wiremo_manual_datetime_start_display")) {
    function wiremo_manual_datetime_start_display() {
        if (current_user_can('administrator')) { ?>
            ...
            ?>
            <input id="wiremo-datetime-start" <?php if(!(get_option('wiremo_widget_display'))) echo "disabled"; ?> type="text" name="wiremo_manual_datetime_start"
...
if(!function_exists("wiremo_manual_datetime_end_display")) {
    function wiremo_manual_datetime_end_display() {
        if (current_user_can('administrator')) { ?>
            ...
            $wiremo_date_time = wiremo_get_orders_datetime();
            ?>
            <input id="wiremo-datetime-end" <?php if(!(get_option('wiremo_widget_display'))) echo "disabled"; ?> type="text" name="wiremo_manual_datetime_end"
...
if(!function_exists("wiremo_manual_create_campaigns_display")) {
    function wiremo_manual_create_campaigns_display() {
        if (current_user_can('administrator')) {
            $wiremo_date_time = wiremo_get_orders_datetime();
            ?>
            <button <?php if(!(get_option('wiremo_widget_display'))) echo "disabled"; ?> data-old-time-orders="<?php if(isset($wiremo_date_time->old_date)) { echo $wiremo_date_time->old_date; } ?>" data-future-time-orders="<?php if(isset($wiremo_date_time->future_date)) { echo $wiremo_date_time->future_date; } ?>" class="wiremo-create-campaigns <?php if(!(get_option('wiremo_widget_display'))) echo " disabled-btn-campaigns "; ?> btn btn-primary"
...
function display_wiremo_panel_fields()
{
    if (current_user_can('administrator')) {
        if (get_option("wiremo_widget_location")) {
            $location = esc_attr(get_option("wiremo_widget_location"));
            ($location == "Footer") ? $style_custom_title_reviews = "wiremo-custom-text-review" : $style_custom_title_reviews = "wiremo-custom-text-review hidden";
            ($location == "Footer") ? $style_custom_tab_name = "wiremo-widget-tab-name hidden" : $style_custom_tab_name = "wiremo-widget-tab-name";
        }
        else {
...
if($wiremoDisableWoo) {
    ...
    { ?>
        ...
        <p style="font-style:italic;color:red;margin-left:5px"><?php __('Product reviews are currently disabled.', 'wiremo-widgett'); ?></p><?php
    }
...
if ($displayWidget && $wiremoDisableWoo) {
    ...
    add_action('woocommerce_single_product_summary', 'wiremo_woocommerce_template_single_rating', 10);
        ...
        if ($location == "Tab") {
            ...
            if(!function_exists("wiremo_product_tab_content")) {
                function wiremo_product_tab_content() {
                    ...
                    wp_add_inline_script('wiremo-widget-script', '!function(){var e=window.wiremo_config?new window.wiremo_config:{},t=Object.assign({permalinkStructure:"'.$str_permalink.'",reviewType:"product",product:'.json_encode($product).',reviewSource:"'.(string)$productId.'",identifier:"'.(string)$productId.'"'.$token.'},e),n=document.createElement("script");n.type="text/javascript",n.async=!0,n.src="'.WRPW_URLWIDGET.'?k='.get_option("wiremo-siteId").'&w="+encodeURIComponent(JSON.stringify(t));var o=document.getElementsByTagName("script")[0];o.parentNode.insertBefore(n,o)}();');
                }
            }
